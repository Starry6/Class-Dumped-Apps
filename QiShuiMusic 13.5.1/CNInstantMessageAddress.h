@interface CNInstantMessageAddress : NSObject
@property (nonatomic) NSString username;
@property (nonatomic) NSString userIdentifier;
@property (nonatomic) NSString service;
@property (nonatomic) NSString teamIdentifier;
@property (nonatomic) NSArray bundleIdentifiers;
@property (nonatomic) Q hash;
@property (nonatomic) # superclass;
@property (nonatomic) NSString description;
@property (nonatomic) NSString debugDescription;
- (BOOL)isValid:;
- (void)setService:;
- (id)username;
- (id)init;
- (id)teamIdentifier;
- (id)userIdentifier;
- (void)setUsername:;
- (id)initWithCoder:;
- (unsigned long long)hash;
- (id)service;
- (void)encodeWithCoder:;
- (void)setUserIdentifier:;
- (void)setTeamIdentifier:;
- (void).cxx_destruct;
- (id)description;
- (id)dictionaryRepresentation;
- (id)mutableCopyWithZone:;
- (BOOL)isEqual:;
- (id)copyWithZone:;
- (id)bundleIdentifiers;
- (void)setBundleIdentifiers:;
- (id)initWithUsername:service:;
- (id)initWithUsername:userIdentifier:service:teamIdentifier:bundleIdentifiers:;
+ (id)localizedStringForKey:;
+ (BOOL)supportsSecureCoding;
+ (id)localizedStringForService:;
+ (id)instantMessageAddressWithDictionaryRepresentation:;
+ (id)Username;
@end
