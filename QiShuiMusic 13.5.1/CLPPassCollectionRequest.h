@interface CLPPassCollectionRequest : PBRequest
@property (nonatomic) CLPMeta meta;
@property (nonatomic) NSMutableArray passLocations;
@property (nonatomic) BOOL hasSignature;
@property (nonatomic) NSData signature;
- (id)meta;
- (BOOL)hasSignature;
- (void)setSignature:;
- (id)signature;
- (Class)responseClass;
- (void)writeTo:;
- (unsigned long long)hash;
- (void)copyTo:;
- (void)setMeta:;
- (void)mergeFrom:;
- (void).cxx_destruct;
- (BOOL)readFrom:;
- (id)description;
- (id)dictionaryRepresentation;
- (unsigned int)requestTypeCode;
- (BOOL)isEqual:;
- (id)copyWithZone:;
- (void)clearPassLocations;
- (void)addPassLocation:;
- (unsigned long long)passLocationsCount;
- (id)passLocationAtIndex:;
- (id)passLocations;
- (void)setPassLocations:;
+ (Class)passLocationType;
@end
