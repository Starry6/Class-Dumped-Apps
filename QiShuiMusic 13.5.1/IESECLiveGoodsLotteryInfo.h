@interface IESECLiveGoodsLotteryInfo : MTLModel
@property (nonatomic) NSString activityID;
@property (nonatomic) NSString winRecordID;
@property (nonatomic) q status;
@property (nonatomic) q taskType;
@property (nonatomic) q lotteryType;
@property (nonatomic) NSString jumpUrl;
@property (nonatomic) NSString buttonLabel;
@property (nonatomic) NSString resultLabel;
@property (nonatomic) NSNumber openTime;
@property (nonatomic) NSNumber buyAbandonTime;
@property (nonatomic) NSNumber serverTime;
@property (nonatomic) NSArray text;
@property (nonatomic) BOOL buttonEnabled;
@property (nonatomic) NSString headerText;
@property (nonatomic) NSString contentText;
@property (nonatomic) IESECCouponButtonModel lotteryTag;
@property (nonatomic) NSString shortTitleText;
@property (nonatomic) Q hash;
@property (nonatomic) # superclass;
@property (nonatomic) NSString description;
@property (nonatomic) NSString debugDescription;
- (void)setResultLabel:;
- (id)jumpUrl;
- (void)setLotteryType:;
- (void)setWinRecordID:;
- (id)buyAbandonTime;
- (void)decreaseSeconds:;
- (id)lotteryTag;
- (long long)lotteryType;
- (id)openTime;
- (void)setBuyAbandonTime:;
- (void)setJumpUrl:;
- (void)setLotteryTag:;
- (void)setOpenTime:;
- (void)setServerTime:;
- (void)setShortTitleText:;
- (id)shortTitleText;
- (id)winRecordID;
- (id)headerText;
- (void)setStatus:;
- (void)setText:;
- (long long)taskType;
- (void).cxx_destruct;
- (id)buttonLabel;
- (long long)status;
- (id)serverTime;
- (id)text;
- (void)setTaskType:;
- (void)setButtonLabel:;
- (BOOL)buttonEnabled;
- (id)activityID;
- (id)contentText;
- (id)resultLabel;
- (void)setActivityID:;
+ (id)lotteryTagJSONTransformer;
+ (id)shortPicJSONTransformer;
+ (id)JSONKeyPathsByPropertyKey;
@end
